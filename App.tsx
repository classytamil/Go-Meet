...
const App: React.FC = () => {
  ...
  // Simulation: Handle Waiting Room Admission
  useEffect(() => {
  ...
  const initMedia = async (isHost: boolean = false) => {
    ...
    try {
      ...
      if (err.name === 'NotAllowedError' || err.name === 'PermissionDeniedError' || errMsg.toLowerCase().includes('denied') || errMsg.toLowerCase().includes('permission')) {
        setMediaError("Permission Denied: Go Meet needs access to your camera and microphone. Please click the camera/lock icon in your browser address bar and set 'Camera' and 'Microphone' to 'Allow'.");
      } else if (err.name === 'NotFoundError' || err.name === 'DevicesNotFoundError') {
        setMediaError("No camera or microphone found. Please connect your devices and try again.");
      } else {
        setMediaError(`Media Error: ${err.message || "Unable to access hardware"}. Please check your system settings.`);
      }
  ...
  const handleToggleScreenShare = async () => {
    ...
    if (!meetingPermissions.allowScreenShare && !isLocalHost) {
      setMediaError("Screen sharing has been disabled by the meeting host.");
    ...
    if (screenStream) {
      ...
      try {
        ...
        try {
          // Attempt sharing with audio first (requires user engagement in dialog)
          ...
          // Fallback if audio fails or is rejected by specific system policies
        ...
        const errMsg = err.message || "";
        if (err.name === 'NotAllowedError' || errMsg.toLowerCase().includes('denied') || errMsg.toLowerCase().includes('permission')) {
          setMediaError("Screen share cancelled. Please try again and allow the browser permission.");
        } else {
          setMediaError("Unable to start screen sharing. Ensure you are on a secure connection (HTTPS).");
        }
      ...
      <div className="min-h-screen bg-[#0f1115] flex flex-col text-white overflow-y-auto">
        ...
        <main className="flex-1 p-6 md:p-12 max-w-6xl mx-auto w-full grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          <div className="space-y-8 text-center lg:text-left">
            ...
            {mediaError && (
              <div className="p-6 bg-red-500/10 border border-red-500/30 rounded-[2rem] text-red-400 text-sm flex flex-col gap-4 animate-in slide-in-from-top-4">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 rounded-2xl bg-red-500/20 flex items-center justify-center text-xl shrink-0"><i className="fas fa-lock"></i></div>
                  <div className="flex flex-col gap-1">
                    <span className="font-bold text-base">Permissions Required</span>
                    <span className="text-xs leading-relaxed opacity-80">{mediaError}</span>
                  </div>
                </div>
                <button onClick={() => initMedia(false)} className="w-full py-3 bg-red-500 hover:bg-red-400 text-white rounded-xl font-bold transition-all active:scale-95 shadow-lg shadow-red-900/20">Retry Permissions</button>
              </div>
            )}
  ...
  if (status === MeetingStatus.WAITING_ROOM) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#0f1115] p-6 text-white text-center">
        <div className="max-w-md w-full glass-panel p-12 rounded-[3rem] space-y-6 border border-blue-500/20 shadow-blue-900/10">
           <div className="relative flex items-center justify-center mx-auto w-24 h-24 mb-6">
              ...
              <div className="relative w-16 h-16 bg-blue-600 rounded-full flex items-center justify-center shadow-xl border border-blue-400/30"><i className="fas fa-user-clock text-2xl"></i></div>
           </div>
           <h1 className="text-3xl font-bold">Verifying identity...</h1>
           <p className="text-gray-400 text-sm leading-relaxed">The host is reviewing your request. You'll be in soon.</p>
           <button onClick={() => window.location.reload()} className="w-full py-4 bg-white/5 hover:bg-white/10 rounded-2xl text-xs font-bold transition-all text-gray-400 border border-white/5 active:scale-95 uppercase tracking-widest">Cancel</button>
        </div>
      </div>
    );
  }
